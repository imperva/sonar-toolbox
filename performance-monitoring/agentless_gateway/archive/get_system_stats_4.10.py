...
from python_commons.environment import set_jsonar_env
from python_commons.sonarw import get_admin_client

set_jsonar_env()
sys.path.insert(0, f'{os.environ["JSONAR_BASEDIR"]}/bin')
...
def main():
    lmrm__globals = get_admin_client('get_system_stats.py').get_database('lmrm__sonarg').get_collection('lmrm__globals')
    uid_display_name = lmrm__globals.find_one({'_id': 'globals'}).get('jsonar_uid_display_name')
    stats = {
        'agentless_gw_name': uid_display_name,
...
def get_cpu_stats():
    ...
    # output = subprocess.run(['sar', '-P', 'ALL', '1', '1'], capture_output=True).stdout.decode('utf-8').strip()
    # sar_values = [line for line in output.split('\n') if re.match(r'\d+:\d+', line)]
    # sar_values = [value.replace(' AM', '').replace(' PM', '') for value in sar_values]

    # index = sar_values[0].replace('%', '').split()
    # for line in sar_values[1:]:
    #     line = line.split()
    #     stats['sar'][line[1]] = {
    #         key: value for key, value in islice(zip(index, line), 2, None)
    #     }
